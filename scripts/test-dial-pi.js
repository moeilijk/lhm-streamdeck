...
function testBuildRefVisibleInPi() {
  ...
  assert(html.includes("3c5e9b5 + V5-prep.12"), "plugin V5 build ref visible in PI");
  assert(html.includes('dial_pi.js?v=V5-prep.12'), "dial PI script is cache-busted with build ref");
...
function testDialViewOptionsAreActionLevel() {
  ...
  const sb = loadDialSandbox({ defaultView, indicatorStyle });
  sb.renderPages = () => {};
  sb.currentSettings = { activeIndex: 0, pages: [{}] };
...
function testBulkGenerationAddsSelectedPreviewPages() {
  const pageSensorSelect = { value: "cpu0" };
  const pageReadingSelect = { value: "1", disabled: false };
  const bulkRule = { value: "sensor-readings" };
  const options = [];
  ...
  const bulkAddBtn = { disabled: true };
  const sb = loadDialSandbox({ pageSensorSelect, pageReadingSelect, bulkRule, bulkPreviewList, bulkAddBtn, pageList: { selectedIndex: 0 } });
  sb.document.createElement = () => ({ selected: false, value: "", textContent: "" });
  sb.renderPages = () => {};
  sb.currentCatalog = {
    sensors: [{ uid: "cpu0", name: "CPU Core #1" }],
    readings: [
      { id: "1", sensorUid: "cpu0", label: "Load", unit: "%", type: "Usage" },
      { id: "2", sensorUid: "cpu0", label: "Clock", unit: "MHz", type: "Clock" },
    ],
  };
  sb.currentSettings = { activeIndex: 0, pages: [] };
  sb.pageSelectionDraft = { sensorUid: "cpu0", readingId: "1" };

  sb.generateBulkPreview();
  assert(options.length === 2, "bulk preview includes all readings from selected sensor");
  assert(bulkAddBtn.disabled === false, "bulk add enabled after preview");
  options[0].selected = false;
  sb.addBulkSelectedPages();
  assert(sb.currentSettings.pages.length === 1, "only selected preview item added");
  assert(sb.currentSettings.pages[0].readingLabel === "Load", "selected reading became a page");
  assert(sb.currentSettings.activeIndex === 0, "active index points at first bulk-added page");
}

function testBulkRulesProduceDistinctCandidateSets() {
  const pageSensorSelect = { value: "cpu0" };
  const pageReadingSelect = { value: "1", disabled: false };
  const bulkRule = { value: "matching-reading" };
  const options = [];
  const bulkPreviewList = {
    innerHTML: "",
    options,
  ...
  sb.currentCatalog = {
    sensors: [
      { uid: "cpu0", name: "CPU Package", category: "cpu" },
      { uid: "cpu1", name: "CPU Package 2", category: "cpu" },
    ],
    readings: [
      { id: "1", sensorUid: "cpu0", label: "CPU Core #1", unit: "%", type: "Load", category: "cpu" },
      { id: "2", sensorUid: "cpu0", label: "CPU Total", unit: "%", type: "Load", category: "cpu" },
      { id: "3", sensorUid: "cpu0", label: "CPU Core #2", unit: "%", type: "Load", category: "cpu" },
      { id: "4", sensorUid: "cpu0", label: "Clock", unit: "MHz", type: "Clock", category: "cpu" },
      { id: "5", sensorUid: "cpu1", label: "CPU Core #1", unit: "%", type: "Load", category: "cpu" },
      { id: "6", sensorUid: "cpu1", label: "CPU Core #2", unit: "%", type: "Load", category: "cpu" },
      { id: "7", sensorUid: "cpu2", label: "CPU Core #1", unit: "W", type: "Power", category: "cpu" },
    ],
  };
  sb.currentSettings = { activeIndex: 0, pages: [] };
  sb.pageSelectionDraft = { sensorUid: "cpu0", readingId: "1" };

  const allReadings = sb.buildBulkCandidates("sensor-readings").map((candidate) => {
    return candidate.sensorUid + ":" + candidate.reading.id + ":" + candidate.reading.label;
  });
  assert(allReadings.join("|") === "cpu0:1:CPU Core #1|cpu0:3:CPU Core #2|cpu0:2:CPU Total|cpu0:4:Clock", "all-readings rule stays on selected sensor only");

  const matchingReadings = sb.buildBulkCandidates("matching-reading").map((candidate) => {
    return candidate.sensorUid + ":" + candidate.reading.id + ":" + candidate.reading.label;
  });
  assert(matchingReadings.join("|") === "cpu1:5:CPU Core #1", "matching-reading rule adds only the selected reading on other sensors");
  assert(!matchingReadings.includes("cpu0:1:CPU Core #1"), "matching-reading excludes selected source reading");
  assert(!matchingReadings.includes("cpu0:2:CPU Total"), "matching-reading excludes CPU Total");
  assert(!matchingReadings.includes("cpu0:3:CPU Core #2"), "matching-reading excludes sibling readings on selected sensor");
  assert(!matchingReadings.includes("cpu1:6:CPU Core #2"), "matching-reading excludes sibling readings on other sensors");
  assert(!matchingReadings.includes("cpu0:4:Clock"), "matching-reading excludes other reading on selected sensor");
  assert(!matchingReadings.includes("cpu2:7:CPU Core #1"), "matching-reading excludes same label with different type/unit");

  const numberedReadings = sb.buildBulkCandidates("matching-numbered-readings").map((candidate) => {
    return candidate.sensorUid + ":" + candidate.reading.id + ":" + candidate.reading.label;
  });
  assert(numberedReadings.join("|") === "cpu0:3:CPU Core #2|cpu1:5:CPU Core #1|cpu1:6:CPU Core #2", "matching-numbered-readings adds numbered siblings except selected source reading");
  assert(!numberedReadings.includes("cpu0:1:CPU Core #1"), "matching-numbered-readings excludes selected source reading");
  assert(!numberedReadings.includes("cpu0:2:CPU Total"), "matching-numbered-readings excludes CPU Total");
  assert(!numberedReadings.includes("cpu0:4:Clock"), "matching-numbered-readings excludes non-numbered readings");
  assert(!numberedReadings.includes("cpu2:7:CPU Core #1"), "matching-numbered-readings excludes same label with different type/unit");

  sb.generateBulkPreview();
  assert(options.length === 1, "matching-reading includes only other-sensor exact selected reading matches");
  assert(options[0].textContent.indexOf("CPU Package 2") !== -1, "preview includes matching reading on another sensor");
  assert(options[0].textContent.indexOf("CPU Core #1") !== -1, "preview keeps selected reading label");
  assert(!options.some((opt) => opt.textContent.indexOf("CPU Core #2") !== -1), "matching-reading does not include sibling readings");
  assert(!options.some((opt) => opt.textContent.indexOf("Clock") !== -1), "matching-reading does not include other readings from the sensor");
  sb.addBulkSelectedPages();
  assert(sb.currentSettings.pages.length === 1, "matching-reading adds exact other-sensor pages");
  assert(sb.currentSettings.pages[0].sensorUid === "cpu1", "matching page uses other sensor");
  assert(sb.currentSettings.pages.every((page) => page.readingLabel === "CPU Core #1"), "only the selected reading is added");
}

function testBulkPreviewInvalidatesWhenInputsChange() {
  const listeners = {};
  const pageSensorSelect = { value: "cpu0", dataset: {}, addEventListener(type, handler) { listeners["sensor:" + type] = handler; } };
  const pageReadingSelect = { value: "1", disabled: false, dataset: {}, addEventListener(type, handler) { listeners["reading:" + type] = handler; } };
  const bulkRule = { value: "sensor-readings", dataset: {}, addEventListener(type, handler) { listeners["rule:" + type] = handler; } };
  const bulkPreviewBtn = { disabled: true, dataset: {}, addEventListener(type, handler) { listeners["preview:" + type] = handler; } };
  const bulkAddBtn = { disabled: true, dataset: {}, addEventListener(type, handler) { listeners["add:" + type] = handler; } };
  const options = [];
  const bulkPreviewList = {
    options,
    appendChild(opt) { options.push(opt); },
  ...
  const sb = loadDialSandbox({
    ...
    bulkPreviewList,
    pageList: { selectedIndex: 0 },
    removePageBtn: { disabled: false },
    moveUpBtn: { disabled: false },
    moveDownBtn: { disabled: false },
    addPageBtn: { disabled: false },
  });
  sb.document.createElement = () => ({ selected: false, value: "", textContent: "" });
  sb.populateSelectedPageReadings = () => {};
  sb.currentCatalog = {
    sensors: [{ uid: "cpu0", name: "CPU Package" }, { uid: "cpu1", name: "CPU Package 2" }],
    readings: [
      { id: "1", sensorUid: "cpu0", label: "Load", unit: "%", type: "Load" },
      { id: "2", sensorUid: "cpu0", label: "Clock", unit: "MHz", type: "Clock" },
      { id: "3", sensorUid: "cpu1", label: "Load", unit: "%", type: "Load" },
    ],
  };
  sb.currentSettings = { activeIndex: 0, pages: [] };
  sb.pageSelectionDraft = { sensorUid: "cpu0", readingId: "1" };
  ...
  sb.bindBulkControls();
  sb.bindSelectedPageSelection();
  listeners["preview:click"]({ preventDefault() {} });
  assert(options.length === 2, "all-readings preview is populated before invalidation");
  assert(bulkAddBtn.disabled === false, "bulk add enabled after preview");

  bulkRule.value = "matching-reading";
  listeners["rule:change"]();
  assert(options.length === 0, "rule change clears stale bulk preview");
  assert(bulkAddBtn.disabled === true, "rule change disables stale bulk add");
  listeners["add:click"]({ preventDefault() {} });
  assert(sb.currentSettings.pages.length === 0, "stale preview cannot be added after rule change");

  listeners["preview:click"]({ preventDefault() {} });
  assert(options.length === 1, "matching-reading preview can be regenerated after invalidation");
  pageReadingSelect.value = "2";
  listeners["reading:change"]();
  assert(options.length === 0, "reading change clears stale bulk preview");
  assert(bulkAddBtn.disabled === true, "reading change disables stale bulk add");
  listeners["add:click"]({ preventDefault() {} });
  assert(sb.currentSettings.pages.length === 0, "stale preview cannot be added after reading change");
}

function optionTexts(options) {
  return options.map((opt) => opt.textContent).join("|");
}

function testBulkAddCompleteFlowForAllRules() {
  const listeners = {};
  const pageSensorSelect = { value: "cpu0", dataset: {}, addEventListener(type, handler) { listeners["sensor:" + type] = handler; } };
  const pageReadingSelect = { value: "1", disabled: false, dataset: {}, addEventListener(type, handler) { listeners["reading:" + type] = handler; } };
  const bulkRule = { value: "sensor-readings", dataset: {}, addEventListener(type, handler) { listeners["rule:" + type] = handler; } };
  ...
  sb.currentCatalog = {
    ...
    readings: [
      { id: "1", sensorUid: "cpu0", label: "CPU Core #1", unit: "%", type: "Load" },
      { id: "2", sensorUid: "cpu0", label: "CPU Core #2", unit: "%", type: "Load" },
      { id: "3", sensorUid: "cpu0", label: "CPU Total", unit: "%", type: "Load" },
      { id: "4", sensorUid: "cpu0", label: "Clock", unit: "MHz", type: "Clock" },
      { id: "5", sensorUid: "gpu0", label: "CPU Core #1", unit: "%", type: "Load" },
      { id: "6", sensorUid: "gpu0", label: "CPU Core #2", unit: "%", type: "Load" },
      { id: "7", sensorUid: "gpu0", label: "CPU Core #1", unit: "W", type: "Power" },
    ],
  };
  sb.currentSettings = { activeIndex: 0, pages: [] };
  sb.pageSelectionDraft = { sensorUid: "cpu0", readingId: "1" };
  ...
  listeners["preview:click"]({ preventDefault() {} });
  assert(options.length === 4, "all-readings preview includes every reading from selected sensor");
  assert(optionTexts(options) === "CPU — CPU Core #1|CPU — CPU Core #2|CPU — CPU Total|CPU — Clock", "all-readings preview content");
  options[2].selected = false;
  options[3].selected = false;
  listeners["add:click"]({ preventDefault() {} });
  assert(sb.currentSettings.pages.length === 2, "all-readings adds only selected preview rows");
  assert(sb.currentSettings.pages.map((p) => p.readingLabel).join("|") === "CPU Core #1|CPU Core #2", "all-readings added page labels");
  assert(sb.currentSettings.pages.every((p) => p.sensorUid === "cpu0"), "all-readings added only selected-sensor pages");
  assert(options.length === 0 && bulkAddBtn.disabled === true, "all-readings add clears preview");

  bulkRule.value = "matching-reading";
  listeners["rule:change"]();
  listeners["preview:click"]({ preventDefault() {} });
  assert(options.length === 1, "matching-reading preview excludes already configured pages");
  assert(optionTexts(options) === "GPU — CPU Core #1", "matching-reading preview content");
  assert(!optionTexts(options).includes("CPU Total"), "matching-reading excludes CPU Total");
  assert(!optionTexts(options).includes("CPU Core #2"), "matching-reading excludes sibling readings");
  assert(!optionTexts(options).includes("Clock"), "matching-reading excludes other reading kinds");
  listeners["add:click"]({ preventDefault() {} });
  assert(sb.currentSettings.pages.length === 3, "matching-reading appends selected preview rows");
  assert(sb.currentSettings.pages.slice(2).map((p) => p.sensorUid + ":" + p.readingLabel).join("|") === "gpu0:CPU Core #1", "matching-reading added exact selected rows");
  assert(sb.currentSettings.activeIndex === 2, "matching-reading active index points at first newly added page");

  bulkRule.value = "matching-numbered-readings";
  listeners["rule:change"]();
  listeners["preview:click"]({ preventDefault() {} });
  assert(options.length === 1, "matching-numbered-readings preview excludes already configured pages");
  assert(optionTexts(options) === "GPU — CPU Core #2", "matching-numbered-readings preview content");
  assert(!optionTexts(options).includes("CPU Total"), "matching-numbered-readings excludes CPU Total");
  assert(!optionTexts(options).includes("Clock"), "matching-numbered-readings excludes other reading kinds");
  listeners["add:click"]({ preventDefault() {} });
  assert(sb.currentSettings.pages.length === 4, "matching-numbered-readings appends selected preview rows");
  assert(sb.currentSettings.pages.slice(3).map((p) => p.sensorUid + ":" + p.readingLabel).join("|") === "gpu0:CPU Core #2", "matching-numbered-readings added exact selected rows");
  assert(sb.currentSettings.activeIndex === 3, "matching-numbered-readings active index points at first newly added page");
}

function testBulkCrossSensorRulesExcludeSourceReading() {
  const listeners = {};
  const pageSensorSelect = { value: "cpu0", dataset: {}, addEventListener() {} };
  const pageReadingSelect = { value: "1", disabled: false, dataset: {}, addEventListener() {} };
  const bulkRule = { value: "matching-reading", dataset: {}, addEventListener(type, handler) { listeners["rule:" + type] = handler; } };
  const bulkPreviewBtn = { disabled: false, dataset: {}, addEventListener(type, handler) { listeners["preview:" + type] = handler; } };
  const bulkAddBtn = { disabled: true, dataset: {}, addEventListener(type, handler) { listeners["add:" + type] = handler; } };
  const options = [];
  const bulkPreviewList = {
    options,
  ...
  sb.currentCatalog = {
    ...
    readings: [
      { id: "1", sensorUid: "cpu0", label: "CPU Core #1", unit: "%", type: "Load" },
      { id: "2", sensorUid: "cpu0", label: "CPU Core #2", unit: "%", type: "Load" },
      { id: "3", sensorUid: "cpu1", label: "CPU Core #1", unit: "%", type: "Load" },
      { id: "4", sensorUid: "cpu1", label: "CPU Core #2", unit: "%", type: "Load" },
    ],
  };
  sb.pageSelectionDraft = { sensorUid: "cpu0", readingId: "1" };
  ...
  sb.bindBulkControls();
  listeners["preview:click"]({ preventDefault() {} });
  assert(optionTexts(options) === "CPU B — CPU Core #1", "matching-reading excludes selected source reading from an empty start");

  bulkRule.value = "matching-numbered-readings";
  listeners["rule:change"]();
  listeners["preview:click"]({ preventDefault() {} });
  assert(optionTexts(options) === "CPU A — CPU Core #2|CPU B — CPU Core #1|CPU B — CPU Core #2", "matching-numbered excludes only selected source reading");
}

function testBulkAddSkipsDuplicatesAtAddTime() {
  const listeners = {};
  const pageSensorSelect = { value: "cpu0", dataset: {}, addEventListener() {} };
  const pageReadingSelect = { value: "1", disabled: false, dataset: {}, addEventListener() {} };
  const bulkRule = { value: "sensor-readings", dataset: {}, addEventListener(type, handler) { listeners["rule:" + type] = handler; } };
  ...
  sb.document.createElement = () => ({ selected: false, value: "", textContent: "" });
  sb.renderPages = () => {};
  sb.currentSettings = { sourceProfileId: "profile-a", activeIndex: 0, pages: [] };
  sb.currentCatalog = {
    sensors: [{ uid: "cpu0", name: "CPU" }],
    readings: [
      { id: "1", sensorUid: "cpu0", label: "CPU Core #1", unit: "%", type: "Load" },
      { id: "2", sensorUid: "cpu0", label: "CPU Core #2", unit: "%", type: "Load" },
    ],
  ...
  sb.bindBulkControls();
  listeners["preview:click"]({ preventDefault() {} });
  assert(optionTexts(options) === "CPU — CPU Core #1|CPU — CPU Core #2", "bulk preview starts with both rows");

  sb.currentSettings.pages.push(sb.pageFromReading("cpu0", sb.currentCatalog.readings[0], 0));
  listeners["add:click"]({ preventDefault() {} });

  assert(sb.currentSettings.pages.length === 2, "bulk add skips stale duplicate and adds remaining row");
  assert(sb.currentSettings.pages.map((p) => p.readingLabel).join("|") === "CPU Core #1|CPU Core #2", "bulk add stale duplicate labels");
  assert(sb.currentSettings.pages[0].sourceProfileId === "profile-a" && sb.currentSettings.pages[1].sourceProfileId === "profile-a", "bulk duplicate key keeps source profile");
}
...
const tests = [
  ...
  ["bulk generation adds selected preview pages", testBulkGenerationAddsSelectedPreviewPages],
  ["bulk rules produce distinct candidate sets", testBulkRulesProduceDistinctCandidateSets],
  ["bulk preview invalidates when inputs change", testBulkPreviewInvalidatesWhenInputsChange],
  ["bulk add complete flow for all rules", testBulkAddCompleteFlowForAllRules],
  ["bulk cross-sensor rules exclude source reading", testBulkCrossSensorRulesExcludeSourceReading],
  ["bulk add skips duplicates at add time", testBulkAddSkipsDuplicatesAtAddTime],
